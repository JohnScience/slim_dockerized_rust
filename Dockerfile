...
RUN strip target/x86_64-unknown-linux-musl/release/slim_dockerized_rust

# Use a slim image for running the application
FROM alpine as runtime

# Copy only the compiled binary from the builder stage to this image
COPY --from=builder /app/target/x86_64-unknown-linux-musl/release/slim_dockerized_rust /bin/slim_dockerized_rust

# Specify the command to run when the container starts
CMD ["/bin/slim_dockerized_rust"]
